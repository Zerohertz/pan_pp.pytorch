...
class GlobalResult(object):
    def __init__(self, with_e2e=False):
        ...
        self.det_correct_num_recall = 0    
        self.det_correct_num_precision = 0 
        self.chars_gt = 0                  
        self.chars_det = 0               
...
if __name__ == '__main__':
    result=rrc_evaluation_funcs.main_evaluation(validate_data, cleval_evaluation)
    abc=result['method']['Detection']['hmean']
    
    with open('../' + PARAMS.path + '.csv', 'a', encoding='utf8') as f:
        wr = csv.writer(f)
        wr.writerow([PARAMS.path, result['method']['Detection']['hmean']*100, result['method']['Detection']['precision']*100, result['method']['Detection']['recall']*100])
    print(abc)
